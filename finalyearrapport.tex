...
Le chapitre suivant, \textbf{«\,Préparation du Projet\,»}, formalise les besoins, la méthodologie de développement \textbf{en spirale} par cycles successifs et les environnements techniques retenus pour mener à bien la réalisation de KODA.
	...
	% Figures chapitre II : compactes pour limiter les pages blanches
	\newcommand{\ChTwoFig}[1]{%
		\includegraphics[width=0.88\linewidth,keepaspectratio]{#1}%
	}
	\newcommand{\ChTwoFigMed}[1]{%
		\includegraphics[width=\linewidth,height=0.36\textheight,keepaspectratio]{#1}%
	}
	
	\section{Introduction du chapitre}
	
	Le chapitre~I a posé le \textbf{contexte}, la \textbf{problématique} et la
	\textbf{solution KODA} sous forme de vision. Le présent chapitre~II en est la
	\textbf{traduction ingénierie}~: il formalise ce que le système doit faire
	(besoins), comment il doit se tenir dans le temps (exigences non fonctionnelles),
	comment il est structuré (architecture générale), et comment il a été construit
	progressivement (modèle en spirale et cycles). Chaque besoin identifié ici est
	\textbf{traçable} vers un cycle de réalisation et vers un chapitre technique du
	rapport, afin d'éviter l'écart entre l'intention du début de stage et le prototype
	livré.
	...
	Les besoins fonctionnels (BF) décrivent \textit{ce que} KODA doit offrir à
	l'utilisateur. Ils sont dérivés du chapitre~I, enrichis par les capacités réellement
	implémentées dans les cycles~1 à~5 (workflows n8n, Distributeur, robot, KodaMate).
	
	\begin{table}[H]
		\centering
		\caption{Catalogue des besoins fonctionnels KODA}
		\label{tab:ch2-bf}
		\small
		\renewcommand{\arraystretch}{1.2}
		\begin{tabularx}{\textwidth}{|c|>{\raggedright\arraybackslash}p{3.6cm}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}p{2.2cm}|}
			\hline
			\textbf{Id.} & \textbf{Besoin} & \textbf{Description} & \textbf{Cycle(s)} \\
			\hline
			...
			\hline
			BF03 & Accueil \& identification & Reconnaître un visiteur connu ou inviter un inconnu à se présenter (\texttt{unkonu}) ; accueil personnalisé. & 1, 2, 4 \\
			\hline
	...
	\IfFileExists{public/ch2-besoins-fonctionnels-mermaid.png}{%
		\begin{figure}[htbp]
			\centering
			\ChTwoFig{public/ch2-besoins-fonctionnels-mermaid.png}
			\caption{Synthèse des besoins fonctionnels KODA}
		...
		\IfFileExists{public/ch2-besoins-fonctionnels.png}{%
			\begin{figure}[htbp]
				\centering
				\ChTwoFigMed{public/ch2-besoins-fonctionnels.png}
				\caption{Synthèse des besoins fonctionnels KODA}
				\label{fig:ch2-besoins-f}
			\end{figure}
		}{}}
	...
	Les besoins non fonctionnels (BNF) précisent les \textbf{qualités} attendues du
	système (tableau~\ref{tab:ch2-bnf}).
	
	\begin{table}[H]
		\centering
		\caption{Catalogue des besoins non fonctionnels KODA}
		\label{tab:ch2-bnf}
		\small
		\renewcommand{\arraystretch}{1.2}
		\begin{tabularx}{\textwidth}{|c|>{\raggedright\arraybackslash}p{3.4cm}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}p{2.4cm}|}
			\hline
			\textbf{Id.} & \textbf{Qualité} & \textbf{Exigence} & \textbf{Contrôle} \\
			\hline
	...
	%----------------------------------------------------------------
	\section{Architecture générale et acteurs}
	%----------------------------------------------------------------
	
	L'architecture est \textbf{multicouche} avec le \textbf{Distributeur FastAPI} comme
	hub unique~: KodaMate et le robot passent par lui pour atteindre n8n, PostgreSQL et
	les API cloud (Azure, OpenRouter, SerpAPI, Open-Meteo).
	
	\begin{figure}[htbp]
		\centering
		\ChTwoFig{public/ch2-architecture-systeme-generale.png}
		\caption{Architecture générale du système KODA}
		\label{fig:ch2-architecture-systeme}
	\end{figure}
	
	\begin{table}[htbp]
		\centering
		\caption{Acteurs et rôles dans l'écosystème KODA}
	...
	\IfFileExists{public/ch2-diagramme-acteurs.png}{%
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=\linewidth,keepaspectratio]{public/ch2-diagramme-acteurs.png}
	...
	%----------------------------------------------------------------
	\section{Méthodologie en spirale et planification}
	%----------------------------------------------------------------
	
	Le pilotage repose sur le \textbf{modèle en spirale} (Boehm), et non sur Scrum ni un
	cycle en~V strict~: à chaque tour, on \textbf{planifie}, \textbf{analyse les risques},
	\textbf{développe} un incrément testable, puis \textbf{valide} avant le cycle suivant.
	Cette approche convient à un prototype IA multicouche (n8n, backend, robot, mobile).
	
	\begin{figure}[htbp]
		\centering
		\ChTwoFig{public/ch2-modele-spirale.png}
		\caption{Modèle en spirale et six cycles thématiques KODA}
		\label{fig:ch2-modele-spirale}
	\end{figure}
	
	Le tableau~\ref{tab:cycles_koda} relie chaque \textbf{cycle} au \textbf{chapitre}
	du rapport (chapitre~IV = Cycle~2 backend).
	
	\begin{table}[htbp]
		\centering
		\caption{Cycles, chapitres du rapport et livrables}
		\label{tab:cycles_koda}
		\small
		\renewcommand{\arraystretch}{1.2}
		\begin{tabularx}{\textwidth}{|c|>{\raggedright\arraybackslash}p{2.8cm}|c|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}p{2.4cm}|}
			\hline
			\textbf{Cy.} & \textbf{Thème} & \textbf{Ch.} & \textbf{Livrable} & \textbf{Besoins} \\
			\hline
	...
	Équipe PFE (DSI3.2, ISET Mahadia), binôme, encadrement et lien MicroEdition (ch.~I).
	Dépendances~: cycles 1--2 en parallèle partiel puis intégration ; cycle~3 tôt ;
	cycles~4--5 après backend ; cycle~6 en recette globale.
	
	\begin{figure}[htbp]
		\centering
		\ChTwoFig{public/ch2-tracabilite-besoins.png}
		\caption{Traçabilité besoins $\rightarrow$ cycles de réalisation}
		\label{fig:ch2-tracabilite}
	\end{figure}
	
	\IfFileExists{public/ch2-planification-cycles-mermaid.png}{%
		\begin{figure}[htbp]
			\centering
			\ChTwoFig{public/ch2-planification-cycles-mermaid.png}
			\caption{Planification --- dépendances entre les six cycles}
		...
		\IfFileExists{public/ch2-planification-cycles.png}{%
			\begin{figure}[htbp]
				\centering
				\ChTwoFigMed{public/ch2-planification-cycles.png}
				\caption{Planification des cycles --- jalons et chevauchements}
				\label{fig:ch2-planification-cycles}
			\end{figure}
		}{}}
	...
	\textbf{Matériel}~: PC de développement (8~Go+ RAM), Raspberry Pi, prototype robot
	(audio, caméra, Nextion), smartphone ou émulateur pour KodaMate, accès Internet
	(Supabase, OpenRouter, SerpAPI, Azure).
	
	\begin{figure}[htbp]
		\centering
		\ChTwoFig{public/ch2-environnement-materiel.png}
		\caption{Environnement matériel de test}
	...
	\begin{itemize}
		...
		\item \textbf{Visual Studio Code} ou Visual Studio pour Python, C\# et LaTeX ;
	...
	\begin{itemize}
		...
		\item \textbf{C\# / .NET MAUI~8} pour l'application cross-plateforme KodaMate ;
		\item \textbf{Bash} et utilitaires système sur Raspberry Pi OS.
	\end{itemize}
	
	\IfFileExists{public/ch2-environnement-logiciel-mermaid.png}{%
		\begin{figure}[htbp]
			\centering
			\ChTwoFig{public/ch2-environnement-logiciel-mermaid.png}
			\caption{Environnement logiciel --- stack technique KODA}
			\label{fig:ch2-env-logiciel}
		\end{figure}
	}{%
		\IfFileExists{public/ch2-environnement-logiciel.png}{%
			\begin{figure}[htbp]
				\centering
				\ChTwoFigMed{public/ch2-environnement-logiciel.png}
				\caption{Environnement logiciel --- outils et chaîne technique}
				\label{fig:ch2-env-logiciel}
			\end{figure}
		}{}}
	...
	\textbf{12 besoins fonctionnels} et \textbf{8 besoins non fonctionnels} identifiés,
	une \textbf{architecture générale} centrée sur le Distributeur, une
	\textbf{méthodologie en spirale} avec six cycles tracés vers les chapitres III à
	VII, et les \textbf{environnements} matériel et logiciel du prototype. Les éléments
	qui manquaient initialement dans ce chapitre --- diagramme d'architecture à jour,
	figure de spirale, tableaux de besoins numérotés, traçabilité explicite --- sont
	désormais formalisés ici pour servir de référence unique avant l'entrée dans le
	détail du \textbf{Cycle~1} (chapitre~III, workflows n8n).
  ...
  \IfFileExists{public/block1-serpapi-search-music.png}{%
	...
	\IfFileExists{public/ch3-architecture-cognitive-globale.png}{%
		\kodafigureL[0.78\textwidth]{public/ch3-architecture-cognitive-globale.png}{Architecture cognitive globale de KODA --- interaction entre les trois webhooks}{fig:ch3-architecture-cognitive-globale}
	}{}
	\FloatBarrier
	...
	le développement et la validation du \textbf{Distributeur de Services}, composant
	central qui constitue la \textbf{couche de services} de l'écosystème. Ce cycle
	intervient après le \textbf{Cycle~1} (moteur de réflexion n8n) et précède les cycles
	embarqué (matériel et logiciel Raspberry Pi) et mobile (KodaMate).
	
	\paragraph{Rôle global du Distributeur.}
	Le Distributeur de Services n'est pas qu'un relais technique~: il joue le rôle de
	\textbf{pivot fonctionnel} de KODA. Théoriquement, il assure trois fonctions
	indissociables~:
	\begin{description}[leftmargin=!, style=nextline, itemsep=0.35em]
		\item[\textbf{Unification}] Il traduit des interactions hétérogènes (parole du robot,
		saisie mobile, événements de présence) en un \textbf{langage commun} que le reste
		du système peut traiter~: identité de l'interlocuteur, contexte mémoriel, demande
		formulée.
		\item[\textbf{Personnalisation}] Il garantit que chaque réponse s'appuie sur le
		\textbf{même profil utilisateur}, quelle que soit la voie d'accès~: une question
		posée au robot et une saisie dans l'application mobile alimentent la même mémoire
		et le même moteur de réflexion.
		\item[\textbf{Continuité}] Il maintient la \textbf{expérience utilisateur} dans le
		temps~: accueil proactif lors d'une détection de présence, dialogue vocal, suivi
		depuis le mobile~--- le robot et l'application ne sont que deux \emph{faces} d'un
		même compagnon, et le Distributeur en est le garant.
	\end{description}
	En ce sens, le backend \textbf{matérialise la fonctionnalité globale} de KODA~: il
	assemble des capacités techniques (vocal, visage, persistance, orchestration) pour
	produire une relation utilisateur--robot \textbf{cohérente, mémorisée et évolutive}.
	L'implémentation concrète (FastAPI, PostgreSQL, Azure, n8n) est détaillée dans les
	sections suivantes~; l'introduction se limite ici au \textbf{pourquoi} de ce composant.
	...
	sections~\ref{sec:ch4-services} et~\ref{sec:ch4-endpoints}.
	...
	\textbf{interfaces stables} exposées par ce backend. La
	figure~\ref{fig:ch4-hub-architecture} détaille ce pivot central~: seul le
	Distributeur connaît n8n, PostgreSQL et les services cloud~; robot et mobile
	ne maintiennent qu'une connexion vers lui.
	
	\IfFileExists{public/ch4-hub-architecture.png}{%
		\kodafigureL[0.95\textwidth]{public/ch4-hub-architecture.png}{Cycle~2 --- Distributeur pivot central (composants et flux)}{fig:ch4-hub-architecture}
	}{}
	\FloatBarrier
	...
	\IfFileExists{public/archiipy.png}{%
		\kodafigureL[0.72\textwidth]{public/archiipy.png}{Exécution du programme backend (Distributeur)}{fig:ch4-archiipy}
	}{}
	\FloatBarrier
	...
	\subsection{Service dédié vers le système embarqué (robot)}
	...
	\subsubsection{Canal WebSocket (temps réel)}
	\label{sec:ch4-websocket}
	
	Outre les requêtes HTTP du pipeline vocal, le robot maintient un canal
	\textbf{WebSocket} persistant avec le Distributeur pour les acquittements, le suivi
	d'état et l'envoi de commandes moteur ou d'expressions en temps quasi réel. Le
	protocole repose sur des messages JSON typés (\texttt{status}, \texttt{action},
	\texttt{command\_result})~; en cas de coupure réseau, le client embarqué applique
	une \textbf{reconnexion avec backoff exponentiel} (cf. tableau~\ref{tab:problemes}).
	
	\IfFileExists{public/ch4-websocket-sequence.png}{%
		\kodafigureL[0.95\textwidth]{public/ch4-websocket-sequence.png}{Cycle~2 --- échanges WebSocket robot $\leftrightarrow$ Distributeur (temps réel)}{fig:ch4-websocket-sequence}
	}{}
	\FloatBarrier
	...
	\label{sec:ch4-reconnaissance-faciale}
	
	La reconnaissance faciale constitue le \textbf{service d'identité} du Distributeur.
	Son rôle théorique est de transformer une \textbf{présence physique} devant le robot
	en une \textbf{identité numérique} exploitable par le reste du système~: sans cette
	étape, KODA ne pourrait ni personnaliser la salutation, ni retrouver l'historique
	conversationnel, ni orienter le raisonnement n8n vers le bon profil
	(\texttt{accompagnement}, notes, préférences).
	
	\paragraph{Principe et modèle de données.}
	L'identification repose sur une \textbf{bibliothèque de visages connus}~: à
	l'inscription (depuis l'application mobile ou un flux d'enregistrement), plusieurs
	photos de référence sont capturées et converties en \textbf{encodages vectoriels}
	(128 dimensions) stockés dans la table \texttt{imageuser}, liée à
	\texttt{utilisateur}. Lors d'une interaction, le robot transmet une capture caméra au
	Distributeur via \texttt{POST /api/identify-face}~; le backend détecte le visage,
	calcule son encodage et le compare à la base de référence. L'identité retenue est
	celle dont la \textbf{distance euclidienne} est minimale, sous réserve d'un
	\textbf{seuil de confiance} (0{,}5 dans notre configuration).
	
	\paragraph{Pipeline d'identification.}
	Le traitement suit quatre étapes conceptuelles~:
	\begin{enumerate}[leftmargin=*, itemsep=0.25em]
		\item \textbf{Capture}~: le Raspberry Pi envoie une image (\texttt{multipart})~;
		\item \textbf{Détection et encodage}~: extraction du visage et calcul du vecteur
		128D (\texttt{face\_recognition} / dlib)~;
		\item \textbf{Comparaison}~: recherche du profil le plus proche dans PostgreSQL~;
		\item \textbf{Décision}~: retour du \texttt{username} identifié et d'un score, ou
		de l'état \texttt{unkonu} (personne inconnue) déclenchant un scénario d'accueil
		générique ou d'invitation à se présenter (Flux~3, chapitre~III).
	\end{enumerate}
	Ce pipeline intervient en \textbf{amont} de chaque dialogue vocal (Action~1,
	figure~\ref{fig:ch4-robot-flux1-partA}) et lors de la détection de présence
	(Action~2, figure~\ref{fig:ch4-robot-flux3-partA})~: l'identité conditionne le ton,
	le contenu et le routage vers n8n.
	
	\IfFileExists{public/ch4-face-recognition-pipeline.png}{%
		\kodafigureL[0.92\textwidth]{public/ch4-face-recognition-pipeline.png}{Pipeline de reconnaissance faciale --- de la capture caméra à l'identité utilisateur}{fig:ch4-face-pipeline}
	}{}
	
	\IfFileExists{public/ch4-face-recognition-sequence.png}{%
		\kodafigureL[0.95\textwidth]{public/ch4-face-recognition-sequence.png}{Diagramme de séquence --- identification faciale (robot, Distributeur, base de référence)}{fig:ch4-face-sequence}
	}{}
	
	\IfFileExists{public/dataset.png}{%
		\kodafigure[0.95\textwidth]{public/dataset.png}{Exemple d'images de référence en mémoire (profils utilisateurs)}{fig:ch4-face-dataset}
	}{}
	
	\paragraph{Cas particuliers.}
	Trois situations méritent d'être distinguées~:
	\begin{itemize}[noitemsep]
		\item \textbf{Personne connue}~: match sous le seuil $\rightarrow$ personnalisation
		immédiate (salutation par le prénom, contexte \texttt{accompagnement})~;
		\item \textbf{Personne inconnue}~: aucun match satisfaisant $\rightarrow$ état
		\texttt{unkonu}, accueil générique et possibilité d'enregistrement ultérieur~;
		\item \textbf{Enregistrement}~: création d'un profil \texttt{utilisateur} et de ses
		encodages via les endpoints CRUD (\texttt{/api/users},
		\texttt{/api/user-images}), sans intervention directe du robot sur la base.
	\end{itemize}
	
	\paragraph{Contraintes et intégration.}
	Le calcul d'encodage étant \textbf{CPU-intensif}, le traitement est délégué à un
	\texttt{ThreadPoolExecutor} (\texttt{run\_in\_executor}) afin de ne pas bloquer la
	boucle asynchrone FastAPI (cf. tableau~\ref{tab:problemes}). Les encodages fréquents
	peuvent être mis en cache (Redis) pour réduire la latence sous charge. Le robot ne
	charge \textbf{jamais} la bibliothèque de vision~: seul le Distributeur détient les
	modèles et les références, ce qui respecte le principe du point de liaison unique
	(§~\ref{sec:ch4-fondements}).
	
	\begin{tcolorbox}[colback=green!5!white, colframe=green!50!black,
		title=\textbf{Résultats de validation}]
	...
	\subsection{Service dédié vers l'application mobile (KodaMate)}
	...
	\IfFileExists{public/ch4-mobile-auth.png}{%
		\kodafigureL[0.92\textwidth]{public/ch4-mobile-auth.png}{Authentification mobile --- séquence \texttt{POST /api/auth/login}}{fig:ch4-mobile-auth}
	}{}
	...
	\IfFileExists{public/ch4-mobile-chat.png}{%
		\kodafigureL[0.92\textwidth]{public/ch4-mobile-chat.png}{Chat mobile --- séquence \texttt{POST /api/trigger-n8n}}{fig:ch4-mobile-chat}
	}{}
	...
	\IfFileExists{public/ch4-mobile-settings.png}{%
		\kodafigureL[0.95\textwidth]{public/ch4-mobile-settings.png}{Paramètres, power et historique --- séquences REST KodaMate}{fig:ch4-mobile-settings}
	}{}
	\FloatBarrier
	...
	\subsection{Service dédié vers la base de données (PostgreSQL / Supabase)}
...
figure~\ref{fig:ch4-classes-schema} (§~\ref{sec:ch4-modelisation}).
	...
	\subsection{Service dédié vers le workflow n8n (Cycle~1)}
	...
	\IfFileExists{public/ch4-n8n-sequence.png}{%
		\kodafigureL[0.95\textwidth]{public/ch4-n8n-sequence.png}{Délégation au workflow n8n --- séquence robot ou mobile $\rightarrow$ Distributeur $\rightarrow$ n8n}{fig:ch4-n8n-sequence}
	}{}
	\FloatBarrier
	...
	\IfFileExists{public/ch4-deployment-network.png}{%
		\kodafigureL[0.92\textwidth]{public/ch4-deployment-network.png}{Conflit d'adressage et CORS --- topologie réseau et solutions}{fig:ch4-deployment-network}
	}{}
	\FloatBarrier
...
\end{lstlisting}
	...
	\IfFileExists{public/ch4-audio-pipeline-logiciel.png}{%
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.72\textwidth]{public/ch4-audio-pipeline-logiciel.png}
...
\end{lstlisting}
	...
	% Figures Cycle~5 : captures compactes, diagrammes lisibles sans saturer la page A4
	\newcommand{\ChFiveCapture}[1]{%
		\includegraphics[width=0.34\textwidth,keepaspectratio]{#1}%
	}
	\newcommand{\ChFiveCapturePair}[2]{%
		\begin{minipage}[t]{0.48\textwidth}\centering
			\includegraphics[width=0.92\linewidth,keepaspectratio]{#1}%
		\end{minipage}\hfill
		\begin{minipage}[t]{0.48\textwidth}\centering
			\includegraphics[width=0.92\linewidth,keepaspectratio]{#2}%
		\end{minipage}%
	}
	\newcommand{\ChFiveCaptureTrio}[3]{%
	...
	% Diagrammes : occuper tout l'espace utile de la page (largeur + hauteur max, sans déformation)
	\newcommand{\ChFiveDiagram}[1]{%
		\includegraphics[width=\linewidth,height=0.72\textheight,keepaspectratio]{#1}%
	}
	
	%------------------------------------------------------------------
	\section{Introduction : pourquoi une application compagnon ?}
	%------------------------------------------------------------------
	
	KODA a été conçu comme un \textbf{compagnon de vie} : présence, dialogue,
	mémoire et personnalisation (chapitre~I). Sur le plan matériel et cognitif, les
	cycles précédents ont donné au robot une voix, un corps et une capacité de
	réflexion. Il reste pourtant une attente essentielle côté humain : pouvoir
	\textbf{habiter la relation} avec son compagnon au-delà du salon --- le
	configurer, le suivre, lui écrire, retrouver ce qui a été dit, même lorsque
	l'on n'est pas face au robot.
	
	C'est le \textbf{besoin} que répond \textbf{KodaMate}. L'application n'est pas
	le « cerveau » de KODA : elle est le \textbf{prolongement relationnel} de
	l'écosystème, accessible depuis un téléphone ou un ordinateur. Elle permet à
	l'utilisateur de rester acteur de la vie de son compagnon : lui donner une
	identité, vérifier qu'il est disponible, échanger par écrit lorsque la voix
	n'est pas adaptée, et consulter la mémoire des conversations comme on relirait
	un carnet de bord partagé.
	
	\textbf{Rôle de KodaMate} dans le projet :
	\begin{itemize}
		\item \textbf{Continuité :} maintenir le lien utilisateur--compagnon entre
		deux présences physiques du robot ;
		\item \textbf{Personnalisation :} traduire l'intention « mon KODA à moi » en
		paramètres concrets (nom, langue, caractère) ;
		\item \textbf{Supervision bienveillante :} savoir si le compagnon est actif ou
		au repos, sans confondre affichage local et réalité du système ;
		\item \textbf{Dialogue écrit :} offrir un canal complémentaire à la parole,
		aligné sur la même mémoire que le dialogue vocal ;
		\item \textbf{Confiance d'accès :} n'ouvrir les fonctions sensibles qu'après
		identification de l'utilisateur.
	\end{itemize}
	
	Ces besoins reprennent explicitement ceux formulés au chapitre~II
	(\emph{pilotage à distance}, \emph{dialogue par message}, \emph{personnalisation}).
	KodaMate en est la réponse concrète du Cycle~5.
	
	\begin{tcolorbox}[colback=blue!5!white, colframe=blue!60!black,
		title=\textbf{Objectif du Cycle 5}]
		Offrir une expérience mobile et desktop simple, continue et sécurisée, pour
		configurer, superviser et converser avec KODA --- sans remplacer le robot ni
		dupliquer son intelligence, mais en prolongeant la relation déjà construite
		dans les cycles précédents.
	\end{tcolorbox}
	
	La figure~\ref{fig:ch5-cas-utilisation} résume les capacités attendues du
	point de vue de l'utilisateur ; la plupart supposent une authentification
	préalable, ce qui traduit l'exigence de confiance énoncée ci-dessus.
	
	\begin{figure}[H]
		\centering
		\ChFiveDiagram{public/ch5-cas-utilisation.png}
		\caption{Cycle~5 --- cas d'utilisation de KodaMate (vue utilisateur)}
		\label{fig:ch5-cas-utilisation}
	\end{figure}
	
	%------------------------------------------------------------------
	\section{Place de KodaMate dans l'écosystème}
	%------------------------------------------------------------------
	
	Dans la vision globale du projet, KodaMate occupe la place du \textbf{interlocuteur
	à distance} : l'utilisateur exprime une intention (lire l'état, modifier un
	réglage, poser une question), et le système central --- déjà présenté au
	chapitre~IV --- se charge de la réaliser en s'appuyant sur la mémoire et le
	raisonnement du chapitre~III. Le robot physique reste le lieu de la présence ;
	l'application reste le lieu du \textbf{pilotage et du suivi}.
	
	\begin{figure}[H]
		\centering
		\ChFiveDiagram{public/ch5-place-ecosysteme.png}
		\caption{Cycle~5 --- place de KodaMate entre l'utilisateur, le Distributeur,
			la cognition n8n et le corps du robot}
		\label{fig:ch5-place-ecosysteme}
	\end{figure}
	
	Cette répartition évite de fragmenter la relation : un même profil, une même
	histoire de conversations, que l'échange soit oral sur le robot ou écrit depuis
	le téléphone.
	
	%------------------------------------------------------------------
	\section{Parcours et principes d'expérience}
	%------------------------------------------------------------------
	
	Avant d'aborder chaque écran, il est utile de décrire le \textbf{parcours
	vécu} par l'utilisateur. Celui-ci commence par une phase d'entrée (connexion,
	éventuelle première configuration), puis bascule vers un usage quotidien en
	quatre espaces : accueil, dialogue, historique et paramètres
	(figure~\ref{fig:ch5-parcours-interfaces}).
	
	\begin{figure}[H]
		\centering
		\ChFiveDiagram{public/ch5-parcours-interfaces.png}
		\caption{Cycle~5 --- parcours utilisateur de KodaMate}
		\label{fig:ch5-parcours-interfaces}
	\end{figure}
	
	\begin{table}[H]
		\centering
		\caption{Interfaces KodaMate et rôles utilisateur}
		\label{tab:ch5-perimetre}
		\renewcommand{\arraystretch}{1.25}
		\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{3.6cm}|>{\raggedright\arraybackslash}X|}
			\hline
			\textbf{Interface} & \textbf{Rôle pour l'utilisateur} \\
			\hline
	...
	Chaque onglet de la phase quotidienne recharge ses informations lorsqu'il
	redevient visible, afin de refléter l'état réel du compagnon sans effort
	...
	Cette section décrit chaque écran selon le même fil : \emph{rôle},
	\emph{fonctionnement}, puis \emph{illustration} (capture d'écran lorsque le
	fichier est disponible dans \texttt{public/}).
	
	%------------------------------------------------------------------
	\subsection{Interface de chargement}
	%------------------------------------------------------------------
	
	\textbf{Rôle.} C'est la première impression de KodaMate : un écran sobre avec
	le logo KODA, qui laisse le temps de vérifier si l'utilisateur peut entrer
	directement dans l'application ou doit se reconnecter.
	
	\textbf{Fonctionnement.} Au lancement, l'application contrôle la session
	enregistrée localement, teste la disponibilité du serveur, puis interroge les
	paramètres du robot. Si la chaîne de vérification échoue, la session est
	effacée et l'utilisateur est renvoyé vers la connexion ; si le robot n'a
	jamais été configuré, vers l'écran de configuration ; sinon, vers les onglets
	principaux. L'adresse du serveur peut être saisie ou mémorisée ; en
	développement sur réseau local, elle est normalisée automatiquement (port,
	émulateur Android).
	
	\begin{figure}[H]
		\centering
		\ChFiveDiagram{public/ch5-seq-bootstrap.png}
		\caption{Cycle~5 --- séquence de démarrage et orientation de l'utilisateur}
		\label{fig:ch5-seq-bootstrap}
	\end{figure}
	
	\IfFileExists{public/ch5-ecran-chargement.png}{%
		\begin{figure}[H]
			\centering
			\ChFiveCapture{public/ch5-ecran-chargement.png}
			\caption{Cycle~5 --- interface de chargement KodaMate}
			\label{fig:ch5-ecran-chargement}
		\end{figure}
	}{}
	...
	\textbf{Rôle.} Établir le lien de confiance entre l'utilisateur et le
	Distributeur : sans connexion valide, aucune autre fonction n'est accessible.
	
	\textbf{Fonctionnement.} L'utilisateur saisit l'adresse du Distributeur sur le
	réseau local (IP du poste de développement), peut \emph{tester le serveur}
	avant de se connecter, puis s'authentifie par email et mot de passe. En cas
	d'échec réseau, un message explicite guide la vérification (backend actif,
	même Wi-Fi, adresse IP). Après connexion réussie, l'application mémorise les
	identifiants nécessaires aux appels suivants.
	
	\begin{figure}[H]
		\centering
		\ChFiveCapture{public/ch5-ecran-login.png}
		\caption{Cycle~5 --- connexion et test du serveur Distributeur}
		\label{fig:ch5-ecran-login}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\ChFiveDiagram{public/ch5-seq-auth.png}
		\caption{Cycle~5 --- séquence d'authentification utilisateur}
		\label{fig:ch5-seq-auth}
	\end{figure}
	
	%------------------------------------------------------------------
	\subsection{Interface de configuration du robot}
	%------------------------------------------------------------------
	
	\textbf{Rôle.} Transformer un compte technique en \emph{compagnon nommé} :
	nom, langue, caractère, date de naissance symbolique, etc. Cette étape
	matérialise la personnalisation évoquée au Cycle~2 et prépare le dialogue n8n.
	
	\textbf{Fonctionnement.} L'écran charge les paramètres existants depuis le
	backend, permet de les modifier, puis enregistre l'ensemble en une seule
	sauvegarde. Une fois validée, l'application marque le robot comme configuré et
	autorise l'accès aux onglets quotidiens. Les mêmes champs restent modifiables
	ultérieurement depuis l'écran Paramètres.
	
	\begin{figure}[H]
		\centering
		\ChFiveCapturePair{public/ch5-ecran-setup.png}{public/ch5-ecran-setup-suite.png}
		\caption{Cycle~5 --- configuration initiale : identité du robot (gauche) et
			voix, langue et état d'allumage (droite)}
		\label{fig:ch5-ecran-setup}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\ChFiveDiagram{public/ch5-seq-configuration.png}
		\caption{Cycle~5 --- séquence de lecture et mise à jour de la configuration}
		\label{fig:ch5-seq-configuration}
	\end{figure}
	...
	\textbf{Rôle.} Constituer le \textbf{tableau de bord} du compagnon : en un
	coup d'œil, l'utilisateur voit si KODA est joignable, dans quel état il se
	trouve, et peut agir sur l'alimentation logique (marche/arrêt). La météo
	d'Alger apporte un contexte ambiental léger, distinct du cœur métier mais utile
	pour l'expérience d'accueil.
	
	\textbf{Fonctionnement.} L'écran combine deux sources : le Distributeur pour
	l'état réel du robot (lecture des paramètres, commandes d'allumage/extinction
	confirmées par relecture serveur) et l'API Open-Meteo pour la météo. En cas
	d'indisponibilité réseau, l'interface passe en mode hors ligne plutôt que
	d'afficher un état fictif. Le rafraîchissement intervient à chaque retour sur
	l'onglet.
	
	\begin{figure}[H]
		\centering
		\ChFiveCaptureTrio{public/ch5-ecran-home.png}{public/ch5-ecran-home-deconnecte.png}{public/ch5-ecran-home-actif.png}
		\caption{Cycle~5 --- accueil : veille et environnement (gauche), réseau
			déconnecté (centre), robot actif (droite)}
		\label{fig:ch5-ecran-home}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\ChFiveDiagram{public/ch5-seq-home.png}
		\caption{Cycle~5 --- séquence de supervision : état KODA, météo et marche/arrêt}
		\label{fig:ch5-seq-home}
	\end{figure}
	
	%------------------------------------------------------------------
	\subsection{Interface de dialogue (Chat)}
	%------------------------------------------------------------------
	
	\textbf{Rôle.} Offrir un canal textuel avec KODA, complémentaire de la voix.
	L'utilisateur formule une question ; la réponse s'appuie sur le même
	raisonnement n8n que le Cycle~1, orchestré par le Distributeur.
	
	\textbf{Fonctionnement.} Le message de l'utilisateur apparaît immédiatement
	dans la conversation ; l'application transmet la question au système central,
	qui déclenche l'analyse et la réflexion (chapitre~III), enregistre l'échange
	en mémoire, puis renvoie une réponse formulée pour l'affichage. La capture
	photo prépare l'envoi multimodal ; l'intégration complète avec la
	reconnaissance faciale reste une évolution prévue.
	
	\begin{figure}[H]
		\centering
		\ChFiveCapture{public/ch5-ecran-chat.png}
		\caption{Cycle~5 --- dialogue texte avec KODA}
		\label{fig:ch5-ecran-chat}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\ChFiveDiagram{public/ch5-seq-chat.png}
		\caption{Cycle~5 --- séquence d'envoi d'une question et affichage de la réponse}
		\label{fig:ch5-seq-chat}
	\end{figure}
	
	%------------------------------------------------------------------
	\subsection{Interface d'historique}
	%------------------------------------------------------------------
	
	\textbf{Rôle.} Donner de la \textbf{continuité mémorielle} à l'usage : retrouver
	ce qui a été dit, filtrer par période, rechercher un mot-clé. Cette fonction
	s'aligne sur la mémoire PostgreSQL alimentée par le Cycle~1 et exposée par le
	Distributeur.
	
	\textbf{Fonctionnement.} L'écran charge les dernières conversations depuis le
	backend, propose des filtres temporels (\textit{aujourd'hui}, \textit{hier},
	\textit{cette semaine}, date au choix), puis affine localement par recherche
	textuelle sur la question, la réponse et le type d'échange. Les résultats sont
	regroupés par jour pour faciliter la lecture.
	
	\begin{figure}[H]
		\centering
		\ChFiveCapture{public/ch5-ecran-historique.png}
		\caption{Cycle~5 --- historique des conversations avec filtre et recherche}
		\label{fig:ch5-ecran-historique}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\ChFiveDiagram{public/ch5-seq-historique.png}
		\caption{Cycle~5 --- séquence de consultation de l'historique}
		\label{fig:ch5-seq-historique}
	\end{figure}
	
	%------------------------------------------------------------------
	\subsection{Interface de paramètres}
	%------------------------------------------------------------------
	
	\textbf{Rôle.} Centraliser les réglages durables : profil du robot, préférences
	d'interface, réseau Wi-Fi (simulation ou scan Android), notifications de
	démonstration et déconnexion.
	
	\textbf{Fonctionnement.} Les champs liés au robot sont synchronisés avec le
	backend comme lors de la configuration initiale. Le choix d'un SSID Wi-Fi est
	enregistré localement pour l'indicateur visuel ; sur Android, un scan peut
	lister les réseaux visibles après autorisation de localisation. Certaines
	options (voix, notifications avancées) sont visibles dans l'interface mais
	encore partiellement déconnectées du backend. La déconnexion efface la session
	et le SSID mémorisé pour repartir d'un état neutre.
	
	\begin{figure}[H]
		\centering
		\ChFiveCaptureTrio{public/ch5-ecran-settings-profil.png}{public/ch5-ecran-settings-wifi.png}{public/ch5-ecran-settings-compte.png}
		\caption{Cycle~5 --- paramètres : profil robot (gauche), Wi-Fi et préférences
			(centre), informations et déconnexion (droite)}
		\label{fig:ch5-ecran-settings}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\ChFiveDiagram{public/ch5-seq-wifi.png}
		\caption{Cycle~5 --- séquence de sélection d'un réseau Wi-Fi (simulation locale)}
		\label{fig:ch5-seq-wifi}
	\end{figure}
	
	%------------------------------------------------------------------
	\section{Organisation technique du projet}
	%------------------------------------------------------------------
	
	Au-delà des écrans, le code respecte une séparation \textbf{Views /
	ViewModels / Services / Models}. Le bootstrap (\texttt{App.xaml.cs}) choisit la
	page initiale ; \texttt{AppShell.xaml} porte la navigation par onglets ;
	\texttt{DistributeurApiService} regroupe les appels REST ; \texttt{SessionService}
	et les préférences MAUI conservent un minimum local (session, URL serveur,
	indicateur de configuration, SSID Wi-Fi).
	
	\begin{table}[H]
		\centering
		\caption{Correspondance services principaux et besoins utilisateur}
		\label{tab:ch5-code-architecture}
		\renewcommand{\arraystretch}{1.25}
		\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{4.2cm}|>{\raggedright\arraybackslash}X|}
			\hline
			\textbf{Composant} & \textbf{Usage dans KodaMate} \\
			\hline
			\texttt{DistributeurApiService} & Authentification, paramètres, chat, historique, marche/arrêt. \\
			\hline
			\texttt{SessionService} & Persistance de la session et du statut « robot configuré ». \\
			\hline
			\texttt{WeatherService} & Météo d'accueil via Open-Meteo (Alger). \\
			\hline
			\texttt{WifiNetworkService} & Scan ou liste de démonstration des réseaux. \\
			\hline
			\texttt{MockFirebaseService} & Données de démonstration pour les notifications. \\
			\hline
		\end{tabularx}
	\end{table}
	
	Sur Android, le manifeste autorise réseau, Wi-Fi, localisation et vibration ;
	le trafic HTTP clair facilite les tests en LAN et devra être remplacé par
	HTTPS en déploiement public.
	
	%------------------------------------------------------------------
	\section{Fiabilité et session}
	%------------------------------------------------------------------
	
	La robustesse repose sur des règles simples mais strictes :
	\begin{itemize}
		\item aucun accès aux onglets sans session valide vérifiée au démarrage ;
		\item effacement automatique de la session si le backend ne répond plus ;
		\item délais d'attente sur les appels HTTP (backend et météo) ;
		\item messages d'erreur explicites sur chaque écran ;
		\item confirmation serveur après une commande marche/arrêt ;
		\item normalisation de l'URL en environnement de développement.
	\end{itemize}
	
	Les données locales restent volontairement limitées : elles ne remplacent pas
	la base PostgreSQL, elles permettent seulement de reprendre l'expérience entre
	deux ouvertures de l'application.
	...
	\begin{table}[H]
		\centering
		\caption{Tests de validation par interface}
		\label{tab:ch5-tests}
		\renewcommand{\arraystretch}{1.25}
		\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{3.3cm}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}p{3.2cm}|}
			\hline
			\textbf{Interface} & \textbf{Procédure} & \textbf{Critère OK} \\
			\hline
...
\end{lstlisting}
	...
	Le Cycle~5 achève la boucle utilisateur de l'écosystème KODA : après la
	cognition (Cycle~1), l'orchestration (Cycle~2) et l'incarnation physique
	(Cycles~3--4), \textbf{KodaMate} offre une présence numérique quotidienne.
	Chaque interface répond à un besoin précis --- entrer, configurer, superviser,
	dialoguer, se souvenir, régler --- sans court-circuiter le Distributeur.
	
	Structurée par écran et fondée sur une base MAUI unique, l'application traduit
	en expérience tangible la continuité promise par les chapitres précédents :
	un même compagnon, une même mémoire, plusieurs modalités d'interaction.
	
	\begin{tcolorbox}[colback=blue!5!white, colframe=blue!60!black,
		title=\textbf{Points clés du Cycle 5}]
		\begin{itemize}[noitemsep]
			\item \textbf{Compagnon utilisateur} : canal mobile/desktop complémentaire de la voix
			\item \textbf{Sept interfaces} : chargement, connexion, configuration, accueil, chat, historique, paramètres
			\item \textbf{Passage unique} par le Distributeur ; n8n et PostgreSQL restent centralisés
			\item \textbf{Supervision et dialogue} : état du robot, marche/arrêt, chat texte, mémoire consultable
			\item \textbf{Évolutions identifiées} : notifications temps réel, multimodal photo, HTTPS en production
		\end{itemize}
	\end{tcolorbox}
	...
	%  CONCLUSION GÉNÉRALE
	%==================================================================
	\chapter{Conclusion générale et synthèse du projet}
	\chaptermark{Conclusion générale}
	
	%------------------------------------------------------------------
	\section{Du constat initial à la solution réalisée}
	%------------------------------------------------------------------
	
	Ce rapport a documenté la conception et la réalisation de \textbf{KODA}, un
	compagnon robotique intelligent visant à dépasser les limites des assistants
	génériques~: absence de personnalité durable, froideur des réponses, dépendance
	à des clouds fermés et faible intégration entre la voix, le corps du robot et le
	pilotage utilisateur (chapitre~I).
	
	La réponse apportée repose sur une \textbf{architecture distribuée} articulée
	autour de cinq briques complémentaires, développées selon un \textbf{modèle en
	spirale} (chapitre~II)~: à chaque tour, une partie du système est précisée,
	construite, testée et raccordée au reste, jusqu'à former un écosystème cohérent.
	Le présent chapitre en donne une \textbf{lecture d'ensemble}, pour que le lecteur
	puisse comprendre, en un seul passage, le rôle de chaque cycle et la manière dont
	les composants coopèrent.
	
	%------------------------------------------------------------------
	\section{Vue d'ensemble : les cinq cycles et leurs rôles}
	%------------------------------------------------------------------
	
	Le tableau~\ref{tab:conclusion-cycles} résume la contribution de chaque cycle.
	En dessous, chaque pilier est rappelé dans le langage du besoin utilisateur, puis
	dans celui de la réalisation technique.
	
	\begin{table}[H]
		\centering
		\caption{Synthèse des cycles KODA --- besoin couvert et composant principal}
		\label{tab:conclusion-cycles}
		\renewcommand{\arraystretch}{1.3}
		\begin{tabularx}{\textwidth}{|c|>{\raggedright\arraybackslash}p{2.8cm}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}p{3.4cm}|}
			\hline
			\textbf{N°} & \textbf{Cycle} & \textbf{Besoin adressé} & \textbf{Composant} \\
			\hline
			...
			\hline
			1 & Analyseur et réflexion & Comprendre, mémoriser et prendre l'initiative de parler & Workflows \textbf{n8n} (3 webhooks) \\
			\hline
			2 & Distributeur de services & Unifier les accès données, voix, mobile et robot & Backend \textbf{FastAPI} (pivot central) \\
			\hline
			3 & Embarqué matériel & Donner un corps mobile, expressif et sensoriel au compagnon & \textbf{Raspberry Pi} + \textbf{Arduino} + capteurs/actionneurs \\
			\hline
			4 & Embarqué logiciel & Écouter, parler, afficher, bouger et dialoguer avec le backend & Logiciel \textbf{Python} sur Raspberry Pi \\
			\hline
			5 & Application KodaMate & Configurer, superviser et converser à distance & Application \textbf{.NET MAUI} \\
			\hline
		\end{tabularx}
	\end{table}
	
	\IfFileExists{public/ch5-place-ecosysteme.png}{%
		\begin{figure}[H]
			\centering
			\includegraphics[width=\linewidth,height=0.72\textheight,keepaspectratio]{public/ch5-place-ecosysteme.png}
			\caption{Vue synthétique de l'écosystème KODA au terme du projet}
			\label{fig:conclusion-ecosysteme}
		\end{figure}
	}{%
		\IfFileExists{public/ch2-architecture-systeme.png}{%
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.94\textwidth,height=0.28\textheight,keepaspectratio]{public/ch2-architecture-systeme.png}
				\caption{Architecture multicouche de KODA --- rappel synthétique}
				\label{fig:conclusion-ecosysteme}
			\end{figure}
		}{}}
	
	%------------------------------------------------------------------
	\section{Les cinq piliers du système KODA}
	%------------------------------------------------------------------
	
	\subsection{Le cerveau conversationnel (Cycle~1)}
	
	Le \textbf{Cycle~1} répond à la question~: \emph{comment KODA réfléchit-il et se
	souvient-il ?} Trois workflows n8n structurent la cognition~:
	\begin{itemize}
		\item \textbf{Webhook~1}~: traitement des questions en temps réel (classification
		sémantique, confrontation à l'historique, réponse contextualisée, notes) ;
		\item \textbf{Webhook~2}~: consolidation mémorielle journalière (synthèses
		d'accompagnement) ;
		\item \textbf{Webhook~3}~: spontanéité --- KODA peut \emph{prendre la parole} en
		s'appuyant sur la mémoire consolidée.
	\end{itemize}
	Ce cycle incarne le passage d'un assistant qui \og répond \fg{} à un compagnon qui
	\og réfléchit \fg{}, en s'appuyant sur PostgreSQL et des services d'IA (OpenRouter,
	SerpAPI, etc.). Sans lui, le robot n'aurait ni personnalité évolutive ni continuité
	dans la relation.
	
	\subsection{Le système nerveux central (Cycle~2)}
	
	Le \textbf{Cycle~2} répond à la question~: \emph{comment tout le monde communique-t-il
	sans se perdre ?} Le \textbf{Distributeur FastAPI} est le \textbf{seul pivot} entre
	le robot, l'application mobile, la base de données, n8n et les services cloud
	(Azure pour la voix et la vision, Supabase/PostgreSQL pour la mémoire). Il expose
	un catalogue d'endpoints REST et WebSocket, gère l'authentification, la
	reconnaissance faciale, le pipeline vocal et le déclenchement des workflows.
	
	\textbf{À retenir}~: ni le téléphone ni le Raspberry Pi n'appellent n8n
	directement~; ils passent par le Distributeur, ce qui garantit cohérence des
	données, sécurité et évolutivité.
	
	\subsection{Le corps du compagnon (Cycles~3 et~4)}
	
	Les \textbf{Cycles~3 et~4} répondent à la question~: \emph{comment KODA est-il
	présent physiquement ?}
	
	Le \textbf{Cycle~3} a posé la plateforme matérielle~: architecture biprocesseur
	(Raspberry Pi pour l'intelligence, Arduino pour le temps réel moteur), mobilité,
	capture audio/vidéo, écran Nextion pour l'expression émotionnelle, alimentation et
	tests d'intégration mécanique.
	
	Le \textbf{Cycle~4} a animé ce corps par un logiciel Python asynchrone~: mot de
	réveil (Vosk), écoute par détection d'activité vocale, envoi des requêtes au
	backend, synthèse et lecture audio, commandes de mouvement vers l'Arduino, affichage
	des émotions sur Nextion, diagnostics matériels et arrêt propre. C'est le cycle qui
	transforme l'assemblage électronique en \textbf{compagnon interactif sur site}.
	
	\subsection{La relation à distance (Cycle~5)}
	
	Le \textbf{Cycle~5} répond à la question~: \emph{comment l'utilisateur habite-t-il
	sa relation avec KODA au quotidien, même loin du robot ?} L'application
	\textbf{KodaMate} (.NET MAUI) offre sept espaces fonctionnels (chargement,
	connexion, configuration, accueil, chat, historique, paramètres). Elle prolonge
	les besoins du chapitre~II --- pilotage à distance, dialogue par message,
	personnalisation --- sans dupliquer l'intelligence du Cycle~1.
	
	%------------------------------------------------------------------
	\section{Comment lire l'ensemble : un scénario type}
	%------------------------------------------------------------------
	
	Pour \textbf{comprendre le rapport dans sa globalité}, imaginez un utilisateur qui
	revient chez lui le soir~:
	\begin{enumerate}
		\item Sur son téléphone, il ouvre \textbf{KodaMate}, consulte l'\textbf{accueil}
		(état du robot, météo) et envoie un message dans le \textbf{chat}.
		\item \textbf{KodaMate} transmet la demande au \textbf{Distributeur}, qui enrichit
		le contexte depuis \textbf{PostgreSQL} et déclenche le \textbf{workflow n8n}.
		\item La réponse est enregistrée en mémoire et renvoyée à l'application~; elle
		sera visible plus tard dans l'\textbf{historique}.
		\item Sur place, le \textbf{Raspberry Pi} détecte le mot de réveil, capture la
		voix, obtient la même logique via le backend, fait parler KODA, affiche une
		émotion sur \textbf{Nextion} et peut actionner les \textbf{moteurs} via
		\textbf{Arduino}.
	\end{enumerate}
	
	Ce scénario montre que KODA n'est pas une juxtaposition de modules, mais une
	\textbf{chaîne de présence}~: cognition centralisée, orchestration unique, corps
	local, lien mobile --- chaque cycle couvre un maillon, aucun ne le remplace.
	...
	\begin{tcolorbox}[colback=blue!5!white, colframe=blue!60!black,
		title=\textbf{Synthèse --- ce qu'il faut retenir du rapport}]
		\begin{enumerate}[noitemsep]
			\item \textbf{Problème}~: concevoir un compagnon personnalisé, mémoriel et
			supervisable, au-delà des assistants génériques.
			\item \textbf{Méthode}~: développement en spirale, six cycles thématiques,
			intégration incrémentale et tests à chaque tour.
			\item \textbf{Cognition (C1)}~: trois webhooks n8n --- réponse, mémoire,
			proactivité.
			\item \textbf{Orchestration (C2)}~: Distributeur FastAPI = point d'accès unique
			(robot, mobile, BDD, n8n, cloud).
			\item \textbf{Incarnation (C3--C4)}~: matériel biprocesseur + logiciel embarqué
			vocal et expressif.
			\item \textbf{Relation utilisateur (C5)}~: KodaMate pour configurer, superviser
			et dialoguer à distance.
			\item \textbf{Fil rouge}~: une seule mémoire, une seule personnalité, plusieurs
			modalités d'interaction (voix, texte, présence physique).
		\end{enumerate}
	\end{tcolorbox}
	
	Sur le plan \textbf{ingénierie}, le projet démontre la faisabilité d'un système
	robotique distribué à coût maîtrisé (Raspberry Pi, APIs ouvertes, workflows
	modifiables) tout en respectant une séparation claire des responsabilités. Sur le
	plan \textbf{humain}, il explore une relation d'accompagnement où l'utilisateur
	reste maître de la configuration de son compagnon --- nom, langue, caractère ---
	et peut suivre son activité sans expertise technique.
	
	%------------------------------------------------------------------
	\section{Limites et enseignements}
	%------------------------------------------------------------------
	
	Comme tout prototype de fin d'études, KODA présente des limites assumées, déjà
	identifiées dans les chapitres de cycles~:
	\begin{itemize}
		\item \textbf{Environnement de démonstration}~: tests souvent réalisés en réseau
		local (HTTP, adresse IP manuelle dans KodaMate), ce qui diffère d'un déploiement
		grand public sécurisé (HTTPS, découverte automatique des services).
		\item \textbf{Maturité mobile}~: certaines fonctions (notifications temps réel,
		envoi photo vers la reconnaissance faciale) restent partielles ou simulées.
		\item \textbf{Robustesse embarquée}~: dépendance au backend pour la cognition~;
		modes dégradés limités en cas de coupure prolongée.
		\item \textbf{Validation globale}~: le \textbf{Cycle~6} (tests et validation
		bout-en-bout formalisés) est présent dans la méthodologie du chapitre~II mais
		reste à renforcer par une campagne de recette utilisateur plus systématique.
	\end{itemize}
	
	Ces limites ne remettent pas en cause la \textbf{cohérence architecturale} du
	projet~; elles définissent une feuille de route réaliste pour une industrialisation
	future.
	...
	\begin{itemize}
		\item \textbf{Production}~: HTTPS, durcissement sécurité (JWT, rate limiting),
		déploiement conteneurisé du backend et observabilité (métriques, traces n8n).
		\item \textbf{Expérience utilisateur}~: notifications push depuis le Distributeur
		vers KodaMate, multimodalité complète (photo $\rightarrow$ identification faciale),
		parcours de configuration guidé sans saisie d'IP.
		\item \textbf{Compagnon physique}~: démarrage automatique au boot du Raspberry,
		télémétrie embarquée, réponses de secours hors ligne.
		\item \textbf{Cognition}~: affinage des prompts n8n, évaluation qualitative des
		synthèses mémorielles, tests A/B sur la spontanéité (Webhook~3).
	\end{itemize}
	...
	Ce rapport de stage a présenté \textbf{KODA} dans sa dimension complète~: une
	ambition (compagnon de vie intelligent et personnalisé), une méthode (spirale et
	cycles thématiques), une architecture (n8n, Distributeur, embarqué, mobile,
	PostgreSQL) et une réalisation incrémentale validée par prototypes, tests et
	captures d'interface.
	
	En relisant les chapitres dans l'ordre --- du cadre général (I) à la préparation
	(II), puis des Cycles~1 à~5 --- le lecteur suit le cheminement d'une idée à un
	système opérationnel. \textbf{Le Cycle~1} pense et mémorise~; \textbf{le Cycle~2}
	orchestre~; \textbf{les Cycles~3 et~4} incarnent~; \textbf{le Cycle~5} relie
	l'utilisateur à son compagnon au quotidien. Ensemble, ils composent une réponse
	structurée à la problématique initiale~: offrir un accompagnement plus humain,
	plus personnel et plus maîtrisé que les assistants du marché, tout en gardant
	l'utilisateur au centre de la relation avec son robot.
	
	Le travail réalisé constitue une \textbf{base solide} pour poursuivre le projet
	vers une version démontrable en conditions réelles~; les cycles documentés
	fournissent à la fois une mémoire technique pour l'équipe et une carte lisible
	pour toute personne souhaitant comprendre, en une lecture, l'ensemble de
	l'écosystème KODA.
