...
figure~\ref{fig:ch4-classes-schema} (§~\ref{sec:ch4-modelisation}).
	...
	affichage, toucher et orientation inertielle), le schéma de câblage, puis une campagne de
	...
	Après la première version matérielle, deux améliorations ont été intégrées au
	prototype~: deux capteurs de touche pour introduire un \textbf{sens tactile},
	et un gyroscope MPU5060 branché sur l'Arduino pour renforcer le contrôle de
	la rotation. Ces éléments sont documentés dans ce cycle car ils modifient
	directement le corps physique et le câblage du robot.
	...
	\begin{itemize}
		...
		\item \textbf{Lecture des capteurs}~: acquisition des états des capteurs de
		touche via GPIO ; le gyroscope MPU5060 est lu par l'Arduino, puis exploité
		à travers la liaison USB série.
	...
	%--- Capteurs sensoriels ---
	\subsection{Capteurs Sensoriels Ajoutés~: toucher et orientation}

	\subsubsection{Gyroscope MPU5060 pour la rotation}

	Le \textbf{MPU5060} est ajouté pour améliorer la précision de l'orientation du
	robot pendant les rotations. Sans retour inertiel, une rotation commandée par
	durée reste approximative~: l'angle réellement parcouru dépend de la charge des
	piles, du frottement des chenilles, de l'inertie du châssis et de l'état du sol.
	Le rôle du gyroscope est donc de fournir une mesure physique de la rotation
	effectuée, afin de comparer le mouvement réel avec l'angle demandé.

	Le scénario matériel retenu est le suivant. Le ReSpeaker estime l'angle de
	provenance de la voix grâce à la DOA. Cet angle devient une consigne de rotation
	pour la base mobile~: le Raspberry Pi transmet la consigne à l'Arduino par
	\textbf{USB série}. L'Arduino pilote alors les moteurs \textbf{M1} et
	\textbf{M3}, lit le MPU5060 pendant la rotation, puis arrête ou corrige le
	mouvement lorsque l'angle réellement parcouru se rapproche de la consigne. La
	caméra, fixée sur le torse, se retrouve ainsi mieux orientée vers la personne
	détectée.

	Le module MPU5060 n'est donc pas câblé directement au Raspberry Pi. Il est
	branché sur l'Arduino via le bus \textbf{I2C} de la carte
	(\textbf{SDA} sur A4, \textbf{SCL} sur A5 sur Arduino Uno), avec une
	alimentation fournie par l'Arduino et une masse commune. Ce choix place la
	mesure inertielle au plus près de la commande moteur temps réel~: le Raspberry
	envoie un angle cible, tandis que l'Arduino ferme localement la rotation avec
	le retour du gyroscope.

	\begin{figure}[H]
		\centering
		\IfFileExists{public/gyro.jpg}{%
			\includegraphics[width=0.58\textwidth]{public/gyro.jpg}
		}{%
		}
		\caption{Module gyroscopique MPU5060 utilisé pour améliorer la précision de rotation}
		\label{fig:ch3-gyro}
	\end{figure}

	\begin{figure}[H]
		\centering
		\IfFileExists{public/koda-cycle3-doa-rotation-gyro.png}{%
			\includegraphics[width=0.94\textwidth]{public/koda-cycle3-doa-rotation-gyro.png}
		}{%
			\fbox{\begin{minipage}{0.88\textwidth}\centering\vspace{0.8cm}
			\vspace{0.8cm}\end{minipage}}
		}
		\caption{Relation conceptuelle~: DOA du ReSpeaker $\rightarrow$ consigne Raspberry $\rightarrow$ Arduino (M1, M3 + MPU5060) $\rightarrow$ caméra du torse orientée}
		\label{fig:ch3-schema-doa}
	\end{figure}

	\subsubsection{Capteurs de touche capacitifs}

	Deux \textbf{capteurs de touche capacitifs} sont ajoutés afin de donner au
	robot un sens tactile simple. Chaque capteur fournit une sortie numérique
	(\textit{touché} / \textit{non touché}) reliée directement à une entrée GPIO du
	Raspberry Pi. Le premier capteur est câblé sur \textbf{GPIO~17} et le second
	sur \textbf{GPIO~27}, avec une alimentation logique en \textbf{3{,}3\,V} et une
	masse commune.

	Ces capteurs ne participent pas à l'orientation du robot. Leur rôle est de
	détecter un contact physique volontaire de l'utilisateur. Dans la couche
	logicielle du Cycle~4, ce signal peut devenir une interruption prioritaire~:
	KODA arrête l'action en cours, change d'expression et déclenche une réaction
	de rire pour simuler une réponse de chatouillement.

	\begin{figure}[H]
		\centering
		\IfFileExists{public/touche.jpg}{%
			\includegraphics[width=0.62\textwidth]{public/touche.jpg}
		}{%
		}
		\caption{Capteurs de touche capacitifs ajoutés pour le sens tactile du robot}
		\label{fig:ch3-touche}
	\end{figure}

	\subsection{Sortie Audio~: double chaîne (jack filaire et Bluetooth)}
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{6cm}|}
			...
			caméra, écran Nextion, capteurs tactiles et liaison USB avec l'Arduino) \\
			\hline
	...
	Les ajouts récents enrichissent ce schéma avec deux capteurs de touche reliés
	aux GPIO du Raspberry Pi et un gyroscope MPU5060 relié à l'Arduino pour le
	suivi inertiel de la rotation.
	...
	\IfFileExists{public/koda-cycle3-hardware.png}{%
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.96\textwidth]{public/koda-cycle3-hardware.png}
			\caption{Architecture matérielle logique mise à jour avec toucher et gyroscope}
			\label{fig:ch3-architecture-materielle-logique}
		\end{figure}
	}{}
	...
	\begin{table}[htbp]
		...
		\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{2.8cm}|>{\raggedright\arraybackslash}p{2.2cm}|>{\raggedright\arraybackslash}X|}
			...
			\hline
			Capteurs de touche & Raspberry Pi & GPIO numérique + alim &
			3{,}3V/GND ; capteur principal sur GPIO~17 ; second capteur sur GPIO~27 \\
			\hline
			Gyroscope MPU5060 & Arduino Uno & I2C + alim &
			VCC/GND Arduino ; SDA$\rightarrow$ A4 ; SCL$\rightarrow$ A5 \\
			\hline
	...
	\begin{itemize}
		...
		\item \textbf{GPIO et I2C}~: les capteurs de touche doivent partager la masse
		du Raspberry Pi et rester en logique 3{,}3\,V ; le gyroscope MPU5060 est
		raccordé au bus I2C de l'Arduino, ce qui impose des fils courts entre le
		module et la carte de commande moteur.
	...
	\begin{table}[htbp]
		...
		\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{2.6cm}|>{\raggedright\arraybackslash}>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}p{3.2cm}|}
			...
			Affichage réactif \\
			\hline
			Capteurs de touche & Lecture des entrées GPIO puis contact physique &
			Passage stable de repos à touché \\
			\hline
			Gyroscope MPU5060 & Lecture I2C par l'Arduino puis retour sur le port série &
			Valeurs cohérentes en rotation, visibles côté Arduino/USB \\
			\hline
...
\end{lstlisting}
	...
	\paragraph{Capteurs de touche (GPIO).}
	Les capteurs tactiles sont vérifiés en lisant directement leurs sorties
	numériques. Le premier capteur est raccordé au \textbf{GPIO~17}; le second
	est raccordé au \textbf{GPIO~27}.

\begin{lstlisting}[style=kodatest, language=Python, caption={Lecture des capteurs de touche sur GPIO}]
...
\end{lstlisting}
	...
	\paragraph{Gyroscope MPU5060 côté Arduino (I2C).}
	Le gyroscope est validé depuis l'Arduino, car c'est cette carte qui le lit
	pendant la rotation. Le test consiste à réveiller le module MPU5060 sur le bus
	I2C de l'Arduino, lire l'axe Z du gyroscope, puis afficher les valeurs sur le
	port série USB.

\begin{lstlisting}[style=kodatest, language=C++, caption={Lecture du MPU5060 par l'Arduino via I2C}]
#include <Wire.h>
...
\begin{lstlisting}[style=kodatest, language=Python, caption={Lecture du retour gyroscope depuis le Raspberry via USB série}]
import serial
...
\end{lstlisting}
	...
	détecte simultanément micro, caméra, écran et capteurs de touche, que
	l'Arduino répond toujours sur USB et renvoie les informations du gyroscope,
	et qu'un scénario court combine~:
	capture vocale, mouvement des servos, rotation des roues, changement
	d'expression Nextion, détection d'un toucher et lecture inertielle, sans coupure
	...
	bidirectionnelle, reconnaissance visuelle, expression émotionnelle, perception
	tactile et orientation plus précise.
	...
	\begin{tcolorbox}[colback=blue!5!white, colframe=blue!60!black,
		...
		\begin{itemize}[noitemsep]
			...
			\item Capteurs ajoutés~: deux touches capacitives pour le toucher et MPU5060 côté Arduino pour la rotation
